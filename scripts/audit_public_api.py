"""Audit the explicit public nyc_geo_toolkit namespace surface."""
...
def discover_public_modules() -> tuple[str, ...]:
    ...
    for child in sorted(PACKAGE_ROOT.iterdir(), key=lambda path: path.name):
        ...
        if child.is_dir() and (child / "__init__.py").exists():
            discovered.append(f"nyc_geo_toolkit.{child.name}")
            continue
        if (
            child.is_file()
            and child.suffix == ".py"
            ...
            discovered.append(f"nyc_geo_toolkit.{child.stem}")
    return tuple(discovered)


def module_path(module_name: str) -> Path | None:
    if not module_name.startswith("nyc_geo_toolkit"):
        return None

    relative = module_name.removeprefix("nyc_geo_toolkit").lstrip(".")
    if not relative:
...
def symbol_origin_module(public_module: str, symbol_name: str, symbol: object) -> str:
    ...
    if symbol_name == "__version__":
        return "nyc_geo_toolkit._version"
...
def main() -> int:
    ...
    lines = [
        "# nyc_geo_toolkit public API audit",
        "",
