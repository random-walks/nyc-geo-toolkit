...
1. Create or verify the PyPI account that will own `nyc-geo-toolkit`, and enable
    2FA.
1. Create or verify a TestPyPI account if you want a dry run before production.
1. Add a pending trusted publisher for project `nyc-geo-toolkit` on both
    TestPyPI and PyPI using:
    - Owner: `random-walks`
    - Repository: `nyc-geo-toolkit`
    - Workflow: `.github/workflows/cd.yml`
    - Environment: `pypi`
1. In GitHub, create the `pypi` environment and add any desired deployment
    protection rules.
1. Set the repository variable `PYPI_PUBLISH_ENABLED=true` only when you are
...
1. Push the tag.
1. Run the `CD` workflow manually from that tag with:
    - `publish=true`
    - `repository=testpypi`
...
1. Publish a GitHub Release from the tag.
